...
func main() {
	...
	for _, arquivo := range arquivosExcel {
		...
		v := strings.Replace(planilha.Cell(22, 2).String(), "$", "", -1)
		fmt.Println(v)
		valorTotal, err := strconv.ParseFloat(strings.TrimSpace(v), 64)
		if err != nil {
	...
	fmt.Printf("O valor total impressas é %.2f ", somaValorTotal)
}
